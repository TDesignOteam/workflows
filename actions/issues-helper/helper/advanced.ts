import type { IIssueCoreEngine, IListIssuesParams, TCloseReason, TCommentInfo, TEmoji, TIssueList, TIssueState, TOutList, TPermissionType } from '../types'
import { EConst } from '../const'
import * as core from '../core'
import { checkPermission, dealStringToArr, matchKeyword, replaceStr2Arr } from '../util'
import {
  doAddAssignees,
  doAddLabels,
  doCloseIssue,
...
export function initAdvancedICE(issueCoreEngine: IIssueCoreEngine) {
  ICE = issueCoreEngine
}
...
export async function doQueryIssues(
  ...
  if (issuesList.length) {
    ...
    for (const issue of issuesList) {
      const bodyCheck = bodyIncludesArr.length > 0
        ? bodyIncludesArr.some(body => (issue.body ?? '').includes(body))
        : true
      ...
      if (excludeLabelsArr.length) {
        if (issue.labels.length) {
          const shouldExclude = issue.labels.some(label => excludeLabelsArr.includes(label.name))
          if (shouldExclude) {
            continue
          }
        }
        else if (excludeLabelsArr.includes(EConst.ExcludeEmpty)) {
          continue
        }
      }

      if (inactiveDay) {
        const lastTime = Date.now() - Number(inactiveDay) * 24 * 60 * 60 * 1000
        let checkTime: number | null = null
        ...
        for (const mode of inactiveMode) {
          ...
          if (mode === 'comment' || mode === 'comment-created') {
            ...
            if (comments.length) {
              checkTime = parseTimestamp(comments[comments.length - 1][mode === 'comment' ? 'updated_at' : 'created_at'])
            }
          }

          if (mode === 'issue-created') {
            checkTime = parseTimestamp(issue.created_at)
          }
        }

        if (!checkTime) {
          checkTime = parseTimestamp(issue.updated_at)
        }
...
export async function doCheckInactive(body: string, emoji?: string) {
  ...
  if (issues.length) {
    ...
    const excludeIssueNumbers = dealStringToArr(core.getInput('exclude-issue-numbers') || '').map(Number)
    const hasInactiveLabelIssueNumbers: number[] = []

    for (const issue of issues) {
      const { labels, number } = issue
      if (excludeIssueNumbers.includes(number)) {
        continue
      }
      ...
      if (!labelNames.includes(inactiveLabel)) {
        ...
        if (body) {
          await doCreateComment(body, emoji, number)
        }
      }
      else {
    ...
    if (hasInactiveLabelIssueNumbers.length) {
      core.info(
        `[doCheckInactive] These issues already has ${inactiveLabel} label! ${JSON.stringify(hasInactiveLabelIssueNumbers)} total ${hasInactiveLabelIssueNumbers.length}`,
      )
    }
  }
  else {
...
export async function doFindIssues() {
  ...
  if (issueList.length) {
    ...
    if (ifCreate) {
      ...
      await doCreateIssue(titleIncludes, bodyIncludes, labels)
    }
  }
...
export async function doLockIssues(body: string, emoji?: string) {
  ...
  if (issues.length) {
    ...
    if (hasLockedIssueNumbers.length) {
      core.info(
        `[doLockIssues] Locked issues ---> ${JSON.stringify(hasLockedIssueNumbers)} total ${hasLockedIssueNumbers.length}`,
      )
    }
  }
  else {
  ...
  if (
    ...
    if (issue.labels.length > 0) {
      newLabels = issue.labels
        .map(({ name }) => name)
        .filter(name => !dealStringToArr(removeLabels).includes(name))
    }
