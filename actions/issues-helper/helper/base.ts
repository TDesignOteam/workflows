import type { IIssueCoreEngine, TCloseReason, TEmoji, TIssueState, TLockReasons, TUpdateMode } from '../types'
import * as core from '../core'
import { dealStringToArr } from '../util'

let ICE!: IIssueCoreEngine

export function initBaseICE(issueCoreEngine: IIssueCoreEngine) {
  ICE = issueCoreEngine
}
...
export async function doCreateCommentEmoji(_commentId: number | void, emoji: string) {
  ...
  if (emoji && commentId) {
    await ICE.createCommentEmoji(Number(commentId), dealStringToArr(emoji) as TEmoji[])
...
export async function doDeleteComment(_commentId: number | void) {
  const commentId = _commentId || core.getInput('comment-id')
  if (commentId) {
    await ICE.deleteComment(Number(commentId))
...
export async function doGetIssue() {
  ...
  core.setOutput('issue-labels', labels.length ? labels.map(({ name }) => name).join(',') : '')
  core.setOutput('issue-assignees', assignees.length ? assignees.map(({ login }) => login).join(',') : '')
}
...
export async function doLockIssue(issueNumber?: number) {
  ...
  if (lockReason && !['off-topic', 'too heated', 'resolved', 'spam'].includes(lockReason)) {
...
export async function doUpdateComment(
  ...
  const commentId = _commentId || core.getInput('comment-id')
  if (commentId) {
    await ICE.updateComment(Number(commentId), body, updateMode)
    core.info(`[doUpdateComment] [${commentId}] success!`)
    if (emoji) {
      await doCreateCommentEmoji(Number(commentId), emoji)
    }
  }
  else {
